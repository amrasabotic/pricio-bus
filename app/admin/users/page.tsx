...
//   async function revokeAccess(userId: string, currentStatus: 'active' | 'inactive') {
//     if (!user) return;
//     setUpdating(userId);
...
//       await supabase.from('pricio_admin_logs').insert({
//         admin_id: user.id,
//         action: newStatus === 'inactive' ? 'revoke_app_access' : 'restore_app_access',
//         target_type: 'user',
//         target_id: userId,
//         metadata: {},
//       });
//       toast.success(newStatus === 'inactive' ? 'Access revoked' : 'Access restored');
//       loadUsers();
//     }
//     setUpdating(null);
//   }
...
//                     <Badge className={u.access?.status === 'active'
//                       ? 'bg-green-100 text-green-700 hover:bg-green-100'
//                       : 'bg-red-100 text-red-600 hover:bg-red-100'
//                     }>
//                       {u.access?.status === 'active' ? 'Active' : 'Revoked'}
//                     </Badge>
//                   </td>
...
//                         size="sm"
//                         variant="ghost"
//                         onClick={() => revokeAccess(u.id, u.access?.status ?? 'inactive')}
//                         disabled={updating === u.id || u.id === user?.id}
//                         className={`gap-1.5 text-xs ${u.access?.status === 'active' ? 'text-slate-400 hover:text-red-500' : 'text-slate-400 hover:text-green-500'}`}
//                       >
//                         <UserX className="w-3.5 h-3.5" />
//                         {u.access?.status === 'active' ? 'Revoke' : 'Restore'}
//                       </Button>
//                     </div>
//                   </td>
//                 </tr>
...
const APP_ID = 'e15e8982-c70b-4507-ac76-b3159f956ec0';
...
export default function AdminUsersPage() {
  ...
  async function loadUsers() {
    ...
    // 1. Get ONLY access for this app
    ...
    // 2. Get profiles ONLY for those users
    ...
    // 4. Include superadmins (not tied to app)
    const { data: superadmins } = await supabase
      .from('profiles')
      .select('*')
      .eq('role', 'superadmin');

    const finalUsers: UserWithAccess[] = [
      ...merged,
      ...(superadmins ?? [])
        .filter((s) => !merged.some((m) => m.id === s.id))
        .map((s) => ({
          ...s,
  ...
  async function revokeAccess(
    ...
    if (error) {
      ...
      toast.success(
        newStatus === 'inactive' ? 'Access revoked' : 'Access restored'
      );
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(val) =>
                          changeRole(
                            u.id,
                            val as 'user' | 'superadmin'
                          )
                        }
                        disabled={
                          updating === u.id || u.id === user?.id
                        }
                      >
                        <SelectTrigger className="h-7 w-32 text-xs">
                          <SelectValue />
